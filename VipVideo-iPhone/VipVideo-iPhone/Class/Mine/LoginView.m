...
+ (void)showLoginViewActionCompleteBlcok:
        (LoginActionCompleteBlock)completeBlock {
...
- (NSString *)getUserPhone {
    return [self.nameField.text isEqualToString:self.pwdField.text] ?
               self.nameField.text :
               @"";
}
...
- (void)setUIFrame {
    ...
    self.titleLabel.font =
        [UIFont systemFontOfSize:29 weight:(UIFontWeightRegular)];
...
- (UILabel *)titleLabel {
    if (!_titleLabel) {
        ...
        _titleLabel.font =
            [UIFont systemFontOfSize:30 weight:(UIFontWeightHeavy)];
...
- (UITextField *)nameField {
    if (!_nameField) {
        ...
        _nameField.font =
            [UIFont systemFontOfSize:18 weight:(UIFontWeightHeavy)];
...
- (UILabel *)descriptionLabel {
    if (!_descriptionLabel) {
        ...
        _descriptionLabel.font =
            [UIFont systemFontOfSize:18 weight:(UIFontWeightThin)];
        _descriptionLabel.textColor     = [UIColor cyanColor];
        _descriptionLabel.numberOfLines = 0;
        _descriptionLabel.text          = @"请"
                                 @"慎重输入手机号，因为该号码是你vip唯一凭证的"
                                 @"重要因素，并且一经提交后便不可再次修改。所"
                                 @"以请确认无误后点击提交按钮";
    }
...
/*
// Only override drawRect: if you pestom drawing.
