...
- (void)viewDidLoad {
    ...
    if ([self.url.absoluteString hasPrefix:@"http"]) {
        ...
        //设置服务器根路径
        NSString *newurl =
            [self.url.absoluteString.stringByDeletingLastPathComponent
        ...
        playerModel.videoURL = [NSURL
            URLWithString:[NSString stringWithFormat:
                                        @"http://localhost:8890/index.m3u8"]];
    }
...
- (void)onDeviceOrientationChange:(NSNotification *)notification {
    //添加是否需要旋转的功能
    NSLog(@"目前播放器是否是锁定的状态 %d", self.wmPlayer.isLockScreen);
...
- (void)zf_playerDownload:(NSString *)url {
    if (url) {
        ...
        [[QSPDownloadTool shareInstance]
            addDownloadTast:url
                      title:self.title ?: self.navigationItem.title
                 andOffLine:YES];
        UIAlertController *alertController = [UIAlertController
            alertControllerWithTitle:@"已添加到下载任务"
                             message:nil
...
- (void)downloadSource:(QSPDownloadSource *)source
          changedStyle:(QSPDownloadSourceStyle)style {
...
/** 控制层即将显示 */
- (void)zf_playerControlViewWillShow:(UIView *)controlView
                        isFullscreen:(BOOL)fullscreen {
    NSLog(@"");
}

/** 控制层即将隐藏 */
- (void)zf_playerControlViewWillHidden:(UIView *)controlView
                          isFullscreen:(BOOL)fullscreen {
}
...
- (void)wmplayer:(WMPlayer *)wmplayer
    clickedFullScreenButton:(UIButton *)fullScreenBtn {
...
- (void)wmplayer:(WMPlayer *)wmplayer
    clickedDownloadButton:(UIButton *)backBtn {
#pragma mark 需要在这里进行存储需要下载的videoUrl数组。然后在在下载完成后删除该URl 并进行排重操作
